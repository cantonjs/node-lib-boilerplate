...
const ask = (question, key, defaultValue) => new Promise((resolve) => {
	const defaults = defaultValue || pkg[key];
...
ask('What is your project name?', 'name', defaultName)
	.then(() => ask('What is description?', 'description', defaultName))
	.then(() => ask('What is repository?', 'repository', `cantonjs/${defaultName}`))
